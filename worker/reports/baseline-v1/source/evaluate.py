...
def main():
    ...
    metadata = {'timestamp_utc': datetime.now(timezone.utc).isoformat(), 'python': platform.python_version(),
                'emails': len(emails), 'upstream_commit': 'fadf77b8db2943427bc46db471650c7f166f40bc',
                ...
                'source_sha256': {name: hashlib.sha256(Path(name).read_bytes()).hexdigest() for name in ('shipping.py', 'documents.py', 'evaluate.py', 'semantic_matching/embeddings.py', 'uv.lock')}}
