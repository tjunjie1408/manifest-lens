...
ALIASES = {
    ...
    'notify_party': ('notify party', 'notify party intermediate consignee', 'notify', 'notify address', 'also notify'),
    ...
    'gross_weight_kg': ('gross weight', 'gross weight kg', 'gross weight kgs', 'gross weight毛重 kgs', 'total gross weight毛重 kgs', 'gross wt', 'gross wt kg', 'gross wt kgs', 'total gross weight', 'total gross weight kg', 'total gross weight kgs', 'total gross wt kgs', 'total gross wt kg', 'weight kg'),
}
...
def canonical(field, raw):
    ...
    # Only whitespace-delimited pipes in entity blocks are layout separators.
...
def extract_fields(rows):
    ...
    for row in rows:
        ...
        if any(key == b or key.startswith(b + ' ') for b in boundaries):
            ...
            continue
        if len(parts) == 2:
            # Address/contact colons belong to the current entity, not a new field.
            if current in ('shipper', 'consignee', 'notify_party') and key in ('p o box', 'po box', 'tel', 'telephone', 'phone', 'fax'):
...
def compare_documents(si, bl):
    ...
    return compare_field_sets(extract_fields(si), extract_fields(bl))


def compare_field_sets(left, right):
    base = {'status': 'NEEDS_REVIEW', 'review_reason': None, 'defect_fields': [], 'has_defect': False}
